...
class TextureCLI:
    ...
    def apply_texture(
        ...
        # Apply texture
        try:
            result_code = self.engine.apply_texture(code, TextureType(texture_type), texture_params)
            ...
            print(f"   Description: {self.engine.get_texture_descriptions().get(texture_type, 'Unknown')}")
            
            # Show visual effects
            visual_effects = self.engine.get_texture_visual_effects(TextureType(texture_type))
            print(f"   Visual: {visual_effects.get('particle_behavior', 'unknown')} ({visual_effects.get('color', 'unknown')})")
...
"""
    ...
    def generate_texture_presets(self, output_file: str = "texture_presets.json"):
        ...
        presets = {
            "musical_dense": {
                ...
                "texture": "dense",
                "params": {
                    "dense_nesting_level": 4,
                    "dense_comprehensions": True
                }
            },
            "minimalist_sparse": {
    ...
    def show_visual_effects(self):
        ...
        for texture_type in TextureType:
            ...
            print(f"   Particle Behavior: {effects.get('particle_behavior', 'unknown')}")
...
Examples:
  ...
  python texture_cli.py visual-effects
    ...
    apply_parser = subparsers.add_parser("apply", help="Apply texture transformation to code")
    ...
    # Texture parameters
    apply_parser.add_argument("--dense-nesting-level", type=int, default=3, help="Dense nesting level")
    apply_parser.add_argument("--dense-comprehensions", action="store_true", help="Use comprehensions")
    apply_parser.add_argument("--sparse-minimal", action="store_true", help="Keep minimal")
    apply_parser.add_argument("--sparse-flat", action="store_true", help="Keep flat")
    apply_parser.add_argument("--smooth-vectorized", action="store_true", help="Use vectorized ops")
    apply_parser.add_argument("--smooth-interpolation", action="store_true", help="Use interpolation")
    apply_parser.add_argument("--grainy-jitter", type=float, default=0.1, help="Grainy jitter amount")
    apply_parser.add_argument("--grainy-stepwise", action="store_true", help="Use stepwise processing")
    apply_parser.add_argument("--polyphonic-voices", type=int, default=3, help="Polyphonic voices")
    apply_parser.add_argument("--polyphonic-harmony", action="store_true", help="Add harmony")
    apply_parser.add_argument("--minimal-lines", type=int, default=3, help="Minimal lines")
    apply_parser.add_argument("--maximal-decorators", action="store_true", help="Add decorators")
    apply_parser.add_argument("--maximal-abstractions", action="store_true", help="Add abstractions")
    apply_parser.add_argument("--maximal-parallel", action="store_true", help="Add parallelization")
    apply_parser.add_argument("--fractal-depth", type=int, default=3, help="Fractal depth")
    apply_parser.add_argument("--fractal-branches", type=int, default=2, help="Fractal branches")
    ...
    presets_parser.add_argument("--output", default="texture_presets.json", help="Output file")
