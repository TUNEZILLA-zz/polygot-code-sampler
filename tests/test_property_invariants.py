...
class TestIRInvariants:
    ...
    def test_backend_consistency(self, start, stop, step):
        ...
        assert len(unique_outputs) == len(
            backends
        ), "All backends should generate unique output"
    ...
    def test_parallel_consistency(self, start, stop):
        ...
        for backend in parallel_backends:
            ...
            assert isinstance(
                seq_output, str
            ), f"{backend} sequential should return string"
            assert isinstance(
                par_output, str
            ), f"{backend} parallel should return string"
    ...
    def test_symbol_generation_stability(self):
        ...
        assert all(
            out == rust_outputs[0] for out in rust_outputs
        ), "Rust symbol generation should be stable"
        assert all(
            out == ts_outputs[0] for out in ts_outputs
        ), "TypeScript symbol generation should be stable"
    ...
    def test_function_signature_consistency(self):
        ...
        assert (
            "pub fn test_function()" in rust_output
        ), "Rust should have pub fn signature"
        assert (
            "export function test_function()" in ts_output
        ), "TypeScript should have export function signature"
