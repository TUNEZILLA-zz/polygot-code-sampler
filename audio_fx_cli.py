...
from pcs.audio_fx import AudioFxEngine, FxSpec
...
class AudioFxCLI:
    ...
    def apply_effects(self, code: str, fx_chain: List[str], params: Dict[str, Any], output_file: Optional[str] = None):
...
"""
        ...
        demos = [
            ...
            },
            {
                "name": "LFO Modulation",
                "fx": ["lfo"],
                "params": {"lfo_target": "param:alpha", "lfo_freq": 0.5, "lfo_depth": 0.3}
            },
            {
                "name": "Swing Groove",
                "fx": ["swing"],
                "params": {"swing_ratio": 0.6}
            },
            {
                "name": "Gate Filter",
                "fx": ["gate"],
...
Examples:
  ...
  python audio_fx_cli.py presets
    ...
    apply_parser.add_argument('--fx', required=True, help='Comma-separated list of effects')
    ...
    apply_parser.add_argument('--reverb-decay', type=float, default=0.6, help='Reverb decay')
    apply_parser.add_argument('--delay-steps', type=int, default=3, help='Delay steps')
    apply_parser.add_argument('--delay-extend-loop', action='store_true', help='Extend loop for delay')
    apply_parser.add_argument('--chorus-voices', type=int, default=3, help='Chorus voices')
    apply_parser.add_argument('--chorus-jitter', type=float, default=0.05, help='Chorus jitter')
    apply_parser.add_argument('--distortion-gain', type=float, default=2.5, help='Distortion gain')
    apply_parser.add_argument('--distortion-clip', type=float, default=1.0, help='Distortion clip')
    apply_parser.add_argument('--lfo-target', default='param:alpha', help='LFO target')
    apply_parser.add_argument('--lfo-freq', type=float, default=0.5, help='LFO frequency')
    apply_parser.add_argument('--lfo-depth', type=float, default=0.3, help='LFO depth')
    apply_parser.add_argument('--swing-ratio', type=float, default=0.6, help='Swing ratio')
    apply_parser.add_argument('--gate-threshold', type=float, default=0.1, help='Gate threshold')
    
    # Demo command
    subparsers.add_parser('demo', help='Demonstrate audio effects')
    
    # Presets command
    presets_parser = subparsers.add_parser('presets', help='Generate audio effects presets')
    presets_parser.add_argument('--output', default='audio_fx_presets.json', help='Output file')
