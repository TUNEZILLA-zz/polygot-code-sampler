...
class PyToIRNested(ast.NodeVisitor):
    ...
    def parse(self, code: str) -> IRComp:
        ...
        for node in tree.body:
            ...
            if isinstance(node, ast.Assign) and isinstance(node.value, ast.Call) and getattr(node.value.func, "id", None) == "sum":
                ...
                break
            # Expr: sum(...)
            if isinstance(node, ast.Expr) and isinstance(node.value, ast.Call) and getattr(node.value.func, "id", None) == "sum":
                gen = node.value.args[0]
                self.comp = self._comp_to_ir(gen, reduce_=IRReduce(op="+", initial="0"))
                break
        if not self.comp:
    ...
    def _comp_to_ir(self, comp: Union[ast.ListComp, ast.GeneratorExp], reduce_: Optional[IRReduce]) -> IRComp:
        ...
        return IRComp(generators=gens, element=element_src, reduce=reduce_, provenance=provenance)
...
def cli():
    ap = argparse.ArgumentParser(description="Polyglot Code Sampler - Step 2 (Nested comprehensions → Rust iterators)")
    ap.add_argument("--file", "-f", type=str, help="Path to a Python file containing a list/generator comprehension")
