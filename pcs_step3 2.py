...
def run_demo(int_type="i32", reduce_int="i64"):
    ...
    for code, name in DEMO_CASES:
        ...
        rust_code = render_rust(
            ir, func_name=name, int_type=int_type, reduce_int=reduce_int
        )
        outputs.append(
            {
                "python": code.strip(),
...
def cli():
    ap = argparse.ArgumentParser(
        description="PCS Step 3 — Dict/Set comprehensions + reductions → Rust"
    )
    ap.add_argument(
        "--file",
        "-f",
        type=str,
        help="Path to a Python file containing a comprehension or reduction",
    )
    ...
    ap.add_argument(
        ...
        default="i32",
        choices=["i32", "i64"],
        help="Preferred integer type hint",
    )
    ap.add_argument(
        "--reduce-type",
        type=str,
        default="i64",
        choices=["i32", "i64"],
        help="Type for sum/prod/max/min",
    )
    ...
    rust = render_rust(
        ir, func_name=args.name, int_type=args.int_type, reduce_int=args.reduce_type
    )
