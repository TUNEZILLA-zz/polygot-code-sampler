...
class SceneValidator:
        ...
        if "morph" in scene_data:
            ...
            if isinstance(morph_duration, (int, float)):
                if morph_duration < self.safety_rules["morph_duration_min"]:
                    self.errors.append(
                        f"Morph duration too short: {morph_duration}s < {self.safety_rules['morph_duration_min']}s"
                    )
                elif morph_duration > self.safety_rules["morph_duration_max"]:
                    self.errors.append(
                        f"Morph duration too long: {morph_duration}s > {self.safety_rules['morph_duration_max']}s"
                    )
        ...
        if "duration" in scene_data:
            ...
            if isinstance(duration, (int, float)):
                if duration < self.safety_rules["scene_duration_min"]:
                    self.errors.append(
                        f"Scene duration too short: {duration}s < {self.safety_rules['scene_duration_min']}s"
                    )
                elif duration > self.safety_rules["scene_duration_max"]:
                    self.errors.append(
                        f"Scene duration too long: {duration}s > {self.safety_rules['scene_duration_max']}s"
                    )
            ...
            if isinstance(intensity, (int, float)):
                if intensity > self.safety_rules["intensity_max"]:
                    if not scene_data.get("allow_boost", False):
                        self.errors.append(
                            f"Scene intensity too high: {intensity} > {self.safety_rules['intensity_max']} (requires allow_boost: true)"
                        )
                    else:
                        self.warnings.append(
                            f"High intensity scene: {intensity} (boost allowed)"
                        )
                ...
                if isinstance(fx, dict) and fx.get("type") == "strobe":
                    ...
                    if "rate_hz" in fx:
                        ...
                        if (
                            ...
                            and rate_hz > self.safety_rules["strobe_hz_max"]
                        ):
                            self.errors.append(
                                f"Strobe frequency too high: {rate_hz}Hz > {self.safety_rules['strobe_hz_max']}Hz"
                            )
                    ...
                    if "on_time_ms" in fx:
                        ...
                        if (
                            isinstance(on_time, (int, float))
                            and on_time < self.safety_rules["strobe_on_time_min_ms"]
                        ):
                            self.errors.append(
                                f"Strobe on-time too short: {on_time}ms < {self.safety_rules['strobe_on_time_min_ms']}ms"
                            )
                    ...
                    if "duty_cycle" in fx:
                        ...
                        if (
                            ...
                            and duty_cycle > self.safety_rules["duty_cycle_max"]
                        ):
                            self.errors.append(
                                f"Strobe duty cycle too high: {duty_cycle} > {self.safety_rules['duty_cycle_max']}"
                            )
    ...
    def _check_performance_constraints(self, scene_data: Dict[str, Any]) -> None:
        ...
        if "fx" in scene_data:
            ...
            if fx_count > 8:
                self.warnings.append(
                    f"High FX count: {fx_count} (may impact performance)"
                )
        ...
        if "sidechain" in scene_data and scene_data["sidechain"].get("enabled", False):
            ...
            if len(maps) > 5:
                self.warnings.append(
                    f"High sidechain map count: {len(maps)} (may impact performance)"
                )
    ...
    if args.file:
        ...
        if args.output:
            with open(args.output, "w") as f:
                ...
                f.write(
                    f"Passed: {sum(1 for success, _, _ in results.values() if success)}\n"
                )
                f.write(
                    f"Failed: {sum(1 for success, _, _ in results.values() if not success)}\n"
                )
