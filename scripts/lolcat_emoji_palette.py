...
class LOLcatEmojiPalette:
    ...
    def demo_palettes(self, text="Emoji Palette Demo"):
        ...
        for palette_name, emojis in EMOJI_PALETTES.items():
            ...
            print(
                f"   Emojis: {' '.join(emojis[:5])}{'...' if len(emojis) > 5 else ''}"
            )
...
def main():
    ...
    parser.add_argument(
        "--text", default="Emoji palette test", help="Text to transform"
    )
    ...
    if args.demo:
        ...
        if args.palette:
            ...
            result = palette_system.transform_with_scene(
                args.text, args.scene, seed=1337
            )
