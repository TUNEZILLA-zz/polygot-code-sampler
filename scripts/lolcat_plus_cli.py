...
def main():
    ...
    parser.add_argument(
        "--preset",
        help="Preset name (classic, uwu-rainbow, studio-safe, nyan-march, prismatic-purr)",
    )
    parser.add_argument(
        "--intensity", type=float, default=0.6, help="Overall intensity (0.0-1.0)"
    )
    parser.add_argument(
        "--uwu", type=float, default=0.4, help="UwU-ifier amount (0.0-1.0)"
    )
    parser.add_argument(
        "--chaos", type=float, default=0.2, help="Chaos case amount (0.0-1.0)"
    )
    parser.add_argument(
        "--emoji", type=float, default=0.15, help="Emoji density (0.0-1.0)"
    )
    parser.add_argument(
        "--nyan-trail", type=float, default=0.0, help="Nyan trail amount (0.0-1.0)"
    )
    parser.add_argument(
        "--gradient-phase", type=float, default=0.0, help="Gradient phase (0.0-1.0)"
    )
    parser.add_argument("--seed", type=int, default=1337, help="Random seed")
    parser.add_argument("--mono", action="store_true", help="Monochrome mode")
    parser.add_argument(
        "--reduced-motion", action="store_true", help="Reduced motion mode"
    )
    parser.add_argument("--output", help="Output file (default: stdout)")
    parser.add_argument(
        "--format",
        choices=["text", "ansi", "json"],
        default="ansi",
        help="Output format",
    )
