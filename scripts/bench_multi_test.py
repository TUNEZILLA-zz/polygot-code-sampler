...
def run_backend_benchmark(backend: str, test_case: Dict[str, Any], test_config: Dict[str, Any]) -> List[str]:
...
def run_julia_benchmark(test_case: Dict[str, Any], env: Dict[str, str]) -> List[str]:
    ...
    try:
        ...
        if result.returncode != 0:
            return [create_error_result(env, "julia", test_case['name'], f"Generation failed: {result.stderr}")]
        ...
        julia_result = subprocess.run(julia_cmd, capture_output=True, text=True, env=env)
        ...
        if julia_result.returncode == 0:
            ...
            return [create_error_result(env, "julia", test_case['name'], f"Execution failed: {julia_result.stderr}")]
...
def run_rust_benchmark(test_case: Dict[str, Any], env: Dict[str, str]) -> List[str]:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, env=env)
        if result.returncode != 0:
            return [create_error_result(env, "rust", test_case['name'], f"Generation failed: {result.stderr}")]
        ...
        # Compile
        compile_cmd = ["rustc", "-O", str(gen_file), "-o", str(TARGETDIR / f"{test_case['name']}_rust")]
        ...
        if compile_result.returncode == 0:
            ...
            if run_result.returncode == 0:
                ...
                return [create_error_result(env, "rust", test_case['name'], f"Execution failed: {run_result.stderr}")]
        else:
            return [create_error_result(env, "rust", test_case['name'], f"Compilation failed: {compile_result.stderr}")]
...
def run_go_benchmark(test_case: Dict[str, Any], env: Dict[str, str]) -> List[str]:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, env=env)
        if result.returncode != 0:
            return [create_error_result(env, "go", test_case['name'], f"Generation failed: {result.stderr}")]
        ...
        # Compile
        compile_cmd = ["go", "build", "-o", str(TARGETDIR / f"{test_case['name']}_go"), str(gen_file)]
        compile_result = subprocess.run(compile_cmd, capture_output=True, text=True)
        ...
        if compile_result.returncode == 0:
            ...
            if run_result.returncode == 0:
                ...
                return [create_error_result(env, "go", test_case['name'], f"Execution failed: {run_result.stderr}")]
        else:
            return [create_error_result(env, "go", test_case['name'], f"Compilation failed: {compile_result.stderr}")]
...
def run_ts_benchmark(test_case: Dict[str, Any], env: Dict[str, str]) -> List[str]:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, env=env)
        if result.returncode != 0:
            return [create_error_result(env, "ts", test_case['name'], f"Generation failed: {result.stderr}")]
        ...
        if run_result.returncode == 0:
            ...
            return [create_error_result(env, "ts", test_case['name'], f"Execution failed: {run_result.stderr}")]
...
def run_csharp_benchmark(test_case: Dict[str, Any], env: Dict[str, str]) -> List[str]:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, env=env)
        if result.returncode != 0:
            return [create_error_result(env, "csharp", test_case['name'], f"Generation failed: {result.stderr}")]
        ...
        # Compile
        compile_cmd = ["dotnet", "build", str(gen_file), "-c", "Release", "-o", str(TARGETDIR)]
        compile_result = subprocess.run(compile_cmd, capture_output=True, text=True)
        ...
        if compile_result.returncode == 0:
            ...
            if run_result.returncode == 0:
                ...
                return [create_error_result(env, "csharp", test_case['name'], f"Execution failed: {run_result.stderr}")]
        else:
            return [create_error_result(env, "csharp", test_case['name'], f"Compilation failed: {compile_result.stderr}")]
            
    except Exception as e:
        return [create_error_result(env, "csharp", test_case['name'], str(e))]

def create_benchmark_results(env: Dict[str, str], backend: str, test_case: Dict[str, Any]) -> List[str]:
...
def create_error_result(env: Dict[str, str], backend: str, test_name: str, error_msg: str) -> str:
...
def main():
    ...
    print(f"🎯 Backends: julia, rust, go, ts, csharp")
    ...
    for test_case in test_cases:
        ...
        for backend in backends:
            try:
                ...
                error_result = create_error_result(env, backend, test_case['name'], str(e))
