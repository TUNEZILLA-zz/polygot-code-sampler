...
class A11yHardMode:
    ...
    def apply_hard_limits(self, fx_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "intensity" in adjusted:
            adjusted["intensity"] = min(
                adjusted["intensity"], self.hard_limits["intensity_cap"]
            )

        # Apply chromatic offset limit
        if "chromatic_offset" in adjusted:
            adjusted["chromatic_offset"] = min(
                adjusted["chromatic_offset"], self.hard_limits["chromatic_offset_max"]
            )
        ...
        if "trail_length" in adjusted:
            adjusted["trail_length"] = min(
                adjusted["trail_length"], self.hard_limits["trails_max_frames"]
            )
...
Examples:
  ...
  python3 scripts/a11y_timing_fix.py --test --hard-mode
    ...
    parser.add_argument(
        "--fps", type=float, default=60.0, help="Frame rate for testing"
    )
    parser.add_argument(
        "--hard-mode", action="store_true", help="Enable A11y hard mode"
    )
    parser.add_argument(
        "--jitter-budget", type=float, default=6.0, help="Jitter budget in ms"
    )
