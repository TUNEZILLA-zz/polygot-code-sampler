...
class LOLcatGuardrailTelemetry:
    ...
    def emit_telemetry(self, reason, parameter, value, threshold, context=None):
        ...
        print(
            f"📊 TELEMETRY: {event['event']}{{reason=\"{reason}\", parameter=\"{parameter}\", value={value:.2f}, threshold={threshold:.2f}}}"
        )
        ...
        for param, threshold in self.clamp_thresholds.items():
            if param in params:
                ...
                if value > threshold:
                    ...
                    event = self.emit_telemetry(
                        reason, param, value, threshold, context
                    )
                    events.append(event)
    ...
    def test_motion_watchdog(self):
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            print(
                f"   Trail: {scenario['trail']}, Emoji: {scenario['emoji']}, Chaos: {scenario['chaos']}"
            )
            ...
            if scenario["reduced_motion"]:
                if (
                    scenario["trail"] > 0.1
                    or scenario["emoji"] > 0.05
                    or scenario["chaos"] > 0.1
                ):
                    self.emit_telemetry(
                        "motion_watchdog", "reduced_motion", 1.0, 0.0, scenario
                    )
                else:
    ...
    def generate_grafana_metrics(self):
        ...
        self.emit_telemetry(
            "emoji_exceeded", "emoji", 0.25, 0.2, {"scene": "stage-punch"}
        )
        self.emit_telemetry("trail_exceeded", "trail", 0.7, 0.6, {"scene": "cat-walk"})
        self.emit_telemetry(
            "motion_watchdog", "reduced_motion", 1.0, 0.0, {"scene": "studio-safe"}
        )

        # Generate metrics summary
        total_events = len(self.telemetry_events)
        warning_events = len(
            [e for e in self.telemetry_events if e["severity"] == "warning"]
        )
...
def main():
    ...
    parser.add_argument(
        "--grafana", action="store_true", help="Generate Grafana metrics"
    )
