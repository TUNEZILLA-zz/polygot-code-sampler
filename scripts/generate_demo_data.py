...
"""
import argparse, json, os, random, math, datetime, pathlib
...
def synth_row(backend, test, base_ns, mode, parallel, n, commit, ts_iso, os_name, cpu) -> Dict:
    # scale by backend
...
def main():
    ...
    ap.add_argument("--days", type=int, default=7, help="Number of past days to synthesize (including today).")
    ...
    ap.add_argument("--tests", type=str, default=",".join([t for t,_ in DEFAULT_TESTS]))
    ap.add_argument("--n-list", type=str, default="100000,1000000", help="Comma-separated N sizes.")
    ...
    for delta in range(args.days-1, -1, -1):
        ...
        ts_base = datetime.datetime.combine(day, datetime.time(7, 0), tzinfo=datetime.timezone.utc)
            ...
            for test, base_ns in test_map.items():
                for n in n_list:
                    ...
                    for mode, parallel in [
                        ...
                        if mode == "broadcast" and (test != "sum_even_squares" or n > 200_000):
                            continue
                        commit = f"demo-{day.strftime('%m%d')}-{backend[:2]}{random.randint(100,999)}"
                        ts_iso = (ts_base + datetime.timedelta(minutes=random.randint(0, 540))).isoformat().replace("+00:00","Z")
                        row = synth_row(backend, test, base_ns, mode, parallel, n, commit, ts_iso, args.os, args.cpu)
                        rows.append(row)
