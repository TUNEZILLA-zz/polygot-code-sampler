...
class LOLcatPresetAB:
    ...
    def morph_to_b(self, duration=2.0):
        ...
        print(
            f"😺 Morphing from {self.preset_a} to {self.preset_b} over {duration}s..."
        )
    ...
    def morph_to_a(self, duration=2.0):
        ...
        self.is_morphing = True
        print(
            f"😺 Morphing from {self.preset_b} to {self.preset_a} over {duration}s..."
        )
    ...
    def get_current_params(self):
        ...
        if self.is_morphing:
            ...
            for key in params_a:
                if key in params_b:
                    morphed_params[key] = (
                        params_a[key] + (params_b[key] - params_a[key]) * ease
                    )
                else:
...
def main():
    ...
    parser.add_argument(
        "--morph-duration", type=float, default=2.0, help="Morph duration in seconds"
    )
