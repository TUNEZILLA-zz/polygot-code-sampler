...
class LOLcatPresetDiffLogger:
    ...
    def calculate_diff(self, preset_a, preset_b):
        ...
        for key in all_keys:
            ...
            if val_a != val_b:
                diff[key] = {"from": val_a, "to": val_b, "delta": val_b - val_a}
    ...
    def format_diff_log(self, preset_a, preset_b, diff):
        ...
        for param, change in diff.items():
            ...
            log_lines.append(
                f"  {param}: {from_val:.2f}→{to_val:.2f} ({direction}{abs(delta):.2f})"
            )
    ...
    def generate_pr_diff(self, preset_a, preset_b):
        ...
        for param, change in diff.items():
            ...
            pr_content += (
                f"| `{param}` | {from_val:.2f} | {to_val:.2f} | {delta:+.2f} |\n"
            )
