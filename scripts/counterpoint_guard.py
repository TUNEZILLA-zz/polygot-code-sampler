...
def analyze_voice_motion(voice1_notes: List[Dict], voice2_notes: List[Dict]) -> List[Tuple[int, int, str]]:
    ...
    for i, note1 in enumerate(voice1_sorted):
        for j, note2 in enumerate(voice2_sorted):
            ...
            if (note1['start'] < note2['start'] + note2['dur'] and 
                note2['start'] < note1['start'] + note1['dur']):
                ...
                if i > 0 and j > 0:
                    prev_interval = get_interval(
                        voice1_sorted[i-1]['note'], 
                        voice2_sorted[j-1]['note']
                    )
                    
                    if bad_parallel_int(prev_interval, interval):
                        issues.append((interval, prev_interval, f"Parallel {interval} at {note1['start']:.2f}s"))
    
    return issues


def fix_parallel_interval(notes: List[Dict], target_note: int, adjustment: int) -> List[Dict]:
...
def apply_counterpoint_guard(voices_data: Dict[str, List[Dict[str, Any]]]) -> Tuple[Dict[str, List[Dict[str, Any]]], List[str]]:
    ...
    for i in range(len(voice_names) - 1):
        ...
        if issues:
            log_messages.append(f"🎭 Found {len(issues)} parallel issues between {voice1_name} and {voice2_name}")
            ...
            log_messages.append(f"✅ No parallel issues between {voice1_name} and {voice2_name}")
...
def main():
    ...
    parser.add_argument("--input", default="out/opera/voices_data.json", help="Input voices data file")
    parser.add_argument("--output", default="out/opera/voices_data_fixed.json", help="Output fixed voices data file")
    parser.add_argument("--log", default="out/opera/counterpoint.log", help="Log file for counterpoint fixes")
