...
def main():
    ...
    tests = [
        ...
        (
            ...
            "python3 -c \"import json, yaml, jsonschema; sch=json.load(open('bench/policy.schema.json')); pol=yaml.safe_load(open('bench/policy.yml')); jsonschema.validate(pol, sch); print('Schema OK')\"",
        ),
    ]
    ...
    if not has_demo_data:
        tests.extend(
            [
                ...
                (
                    "K-Anomaly Detection",
                    "python3 scripts/k_anomaly_detector.py --input site/benchmarks.json --k-threshold 0.6",
                ),
            ]
        )
    else:
