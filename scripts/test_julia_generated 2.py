...
def test_julia_code_generation():
    ...
    test_cases = [
        (
            "sum(i*i for i in range(1, 10) if i%2==0)",
            "120",
        ),  # 2*2 + 4*4 + 6*6 + 8*8 = 4 + 16 + 36 + 64 = 120
    ...
    for i, (python_code, expected) in enumerate(test_cases, 1):
        ...
        if _is_julia_available():
            try:
                ...
                # Execute Julia code
                result = subprocess.run(
                    ["julia", temp_file], capture_output=True, text=True, timeout=10
                )
                ...
                print("❌ Julia execution timed out")
                return False
            except Exception as e:
                print(f"❌ Julia execution error: {e}")
                return False
        else:
            print("⚠️  Julia not available, skipping execution test")
        ...
        try:
            ...
            if mode_type == "parallel":
                if "@threads" in julia_code and "threadid()" in julia_code:
                    print("✅ Parallel mode generated correctly")
                else:
                    print("❌ Parallel mode not generated")
                    return False
            elif mode_type == "fallback":
                if "sequential fallback" in julia_code.lower():
                    print("✅ Sequential fallback generated correctly")
                else:
                    print("❌ Sequential fallback not generated")
                    return False
...
def test_broadcast_mode():
    ...
    try:
        ...
        if "ifelse." in julia_code and "sum(" in julia_code:
            print("✅ Broadcast mode generated correctly")
            return True
        else:
            print("❌ Broadcast mode not generated")
            return False
