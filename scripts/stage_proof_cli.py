  ...
  python3 scripts/stage_proof_cli.py --status
    ...
    if args.load:
        ...
        default_scene = SceneConfig(
            ...
            fx=[
                {"type": "distortion", "drive": 0.7, "tone": 0.6, "mix": 0.5, "bypass": False},
                {"type": "chorus", "depth": 0.35, "rate_hz": 0.2, "width": 0.8, "mix": 0.7},
                {"type": "echo", "time_ms": 180, "feedback": 0.35, "color": 0.4, "mix": 0.4}
            ]
        )
    ...
    # Show status
    if args.status or not any([args.load, args.text, args.acceptance_test, args.intensity, 
                              args.blackout, args.white_bloom, args.lightning_flash]):
        status = engine.get_engine_status()
        print(f"\n📊 Stage-Proof Engine Status:")
        print(f"🎛️ Global Intensity: {status.get('global_intensity', 0)}%")
        print(f"🌑 Blackout: {status.get('momentary_buttons', {}).get('blackout', False)}")
        print(f"💡 White Bloom: {status.get('momentary_buttons', {}).get('white_bloom', False)}")
        print(f"⚡ Lightning Flash: {status.get('momentary_buttons', {}).get('lightning_flash', False)}")
