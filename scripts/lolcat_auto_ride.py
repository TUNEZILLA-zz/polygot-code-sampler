...
class LOLcatAutoRide:
    ...
    def auto_ride(self):
        ...
        self.gradient_phase = max(self.gradient_phase_min, 
                                 min(self.gradient_phase_max, self.gradient_phase))
        ...
        self.intensity = max(self.intensity_min, 
                           min(self.intensity_max, self.intensity))
        
        # Alternate direction for next ride
        self.ride_direction *= -1
        
        print(f"😺 Auto-ride: phase={self.gradient_phase:.2f}, uwu={self.uwu:.2f}, intensity={self.intensity:.2f}")
    ...
    def demo_auto_ride(self, text="Auto-Ride Demo", duration=30):
        ...
        try:
            ...
            while time.time() - start_time < duration:
                ...
                next_ride = self.eight_bars_duration - (elapsed % self.eight_bars_duration)
                print(f"\r😺 {elapsed:.1f}s | Next ride in {next_ride:.1f}s | {result['text']}", end="")
...
def main():
    ...
    parser.add_argument("--breathing", action="store_true", help="Simulate show breathing")
