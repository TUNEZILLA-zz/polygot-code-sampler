...
class LOLcatArtifactStampUnifier:
    ...
    def create_unified_artifact(
        self, text, slug, preset, seed, macro, output_dir="out"
    ):
...
"""
    ...
    def generate_comparison_matrix(self, text="Comparison Matrix"):
        ...
        print(f"\n🔍 Filename Pattern:")
        print(
            f"   matrix__preset-<name>__seed-<n>__macro-C<S>-S<P>-M<M>-C<R>__t-<timestamp>.html"
        )
...
def main():
    ...
    parser.add_argument(
        "--comparison", action="store_true", help="Test side-by-side comparison"
    )
    parser.add_argument(
        "--matrix", action="store_true", help="Generate comparison matrix"
    )
    ...
    parser.add_argument(
        "--text", default="Artifact stamp test", help="Text to transform"
    )
    ...
    if args.demo:
        ...
        macro = {"color": 0.0, "space": 0.0, "motion": 0.0, "crunch": 0.0}
        unifier.create_unified_artifact(
            args.text, args.slug, args.preset, args.seed, macro, args.output
        )
    else:
        print(
            "Usage: python3 lolcat_artifact_stamp_unifier.py --demo | --comparison | --matrix | --create"
        )
