...
class LOLcatSidechain:
        ...
        if current_time - self.last_qps_time > 16:  # ~60 FPS
            ...
            if self.qps > 0.5:  # High QPS
                ...
                if current_time - self.last_qps_time < self.qps_ease_out:
                    ease_factor = (
                        1.0 - (current_time - self.last_qps_time) / self.qps_ease_out
                    )
                else:
                    ease_factor = 0.0
    ...
    def demo_sidechain(self, duration=30):
        ...
        try:
            ...
            while time.time() - start_time < duration:
                ...
                # Show status
                print(
                    f"\r😺 QPS: {qps:.2f} | Error: {error_rate:.2f} | P95: {p95:.1f}ms | "
                    f"Phase: {self.gradient_phase:.2f} | Chaos: {self.chaos:.2f} | "
                    f"Emoji: {self.emoji:.2f} | Trail: {self.trail:.2f}",
                    end="",
                )
    ...
    def test_sidechain(self, text="Sidechain test"):
        ...
        print(
            f"   P95: {self.p95:.1f}ms → Emoji: {self.emoji:.2f}, Trail: {self.trail:.2f}"
        )
...
def main():
    ...
    if args.demo:
        ...
        print(
            "Usage: python3 lolcat_sidechain.py --demo | --test [--qps 0.5 --error 0.1 --p95 8.0]"
        )
