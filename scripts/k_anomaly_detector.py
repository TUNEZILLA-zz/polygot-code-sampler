    ...
    for date, day_records in by_date.items():
        ...
        for backend, records in by_backend.items():
            ...
            if previous and current.get("mean_ns") and previous.get("mean_ns"):
                regression_ratio = (
                    current["mean_ns"] - previous["mean_ns"]
                ) / previous["mean_ns"]
...
def format_anomaly_report(anomalies: list[dict[str, Any]]) -> str:
    ...
    for anomaly in anomalies:
        ...
        report.append(
            f"  Backends: {regressed}/{total} showing regressions ({fraction:.1%})"
        )
        report.append("  Likely Cause: Infrastructure issue (runner, toolchain, etc.)")
        report.append("  Recommendation: Check CI runner logs, toolchain versions")
        report.append("")
...
def main():
    ...
    parser.add_argument(
        ...
        "-i",
        type=Path,
        default=Path("site/benchmarks.json"),
