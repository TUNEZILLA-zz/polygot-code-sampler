...
class LOLcatPaletteAutoselect:
    ...
    def transform_with_scene(self, text, scene_name, metrics_link=0.0, **kwargs):
        ...
        print(
            f"😺 Scene: {scene_name} | Metrics Link: {metrics_link:.2f} | Palette: {palette}"
        )
...
def main():
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Demo scene palette mapping"
    )
    parser.add_argument(
        "--test-bias", action="store_true", help="Test metrics link bias"
    )
    ...
    parser.add_argument(
        "--metrics-link", type=float, default=0.5, help="Metrics link value"
    )
    parser.add_argument(
        "--text", default="Palette autoselect test", help="Text to transform"
    )
    ...
    if args.demo:
        ...
        # Single scene test
        result = autoselect.transform_with_scene(
            args.text, args.scene, args.metrics_link, seed=1337
        )
