...
class CrazyStringFX:
    ...
    def apply_fx_chain(
        self, text: str, fx_chain: List[StringFXType], params: StringFXParams
    ) -> str:
    ...
    def create_fx_preset(
        self, name: str, fx_chain: List[StringFXType], params: StringFXParams
    ) -> Dict[str, Any]:
...
Examples:
  ...
  python scripts/crazy_string_fx.py --text "TuneZilla" --fx glitch_colors,stutter
    ...
    # Create parameters
    params = StringFXParams(
        intensity=args.intensity, speed=args.speed, glitch_factor=args.glitch
    )
    ...
    print(
        f"⚙️  Parameters: intensity={params.intensity}, speed={params.speed}, glitch={params.glitch_factor}"
    )
    ...
    if not args.fx:
        ...
        for fx_name, fx_result in demo.items():
            print(
                f"{fx_name:20}: {fx_result[:50]}{'...' if len(fx_result) > 50 else ''}"
            )
