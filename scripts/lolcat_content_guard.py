...
def test_content_guard():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        result_unguarded = lolcat_plus(
            test_case["text"], content_guard=False, seed=1337
        )
        ...
        if test_case["name"] != "No protection needed":
            if result_guarded["text"] != result_unguarded["text"]:
                ...
                print(
                    "   ❌ Content guard not working - protected content was transformed"
                )
        else:
            if result_guarded["text"] == result_unguarded["text"]:
...
def main():
    ...
    parser.add_argument(
        "--docs", action="store_true", help="Test documentation preservation"
    )
    parser.add_argument(
        "--text",
        default="Test with `code` and https://example.com",
        help="Text to test",
    )
    parser.add_argument(
        "--no-guard", action="store_true", help="Test without content guard"
    )
