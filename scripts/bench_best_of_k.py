...
def run_all_benchmarks(k: int = 5) -> List[Dict[str, Any]]:
    ...
    for test_name, test_func, n in benchmarks:
        ...
        try:
            ...
            print(f"   ✅ Best time: {stats['mean_ns']:,.0f} ns (from {stats['reps']} runs)")
...
def main():
    ...
    parser.add_argument("--k", type=int, default=5, help="Number of runs per test (default: 5)")
    parser.add_argument("--output", "-o", type=Path, default=Path("bench_best_of_k.json"),
