...
def set_scene(name, seconds=0.0, curve="EaseInOut"):
    return post(
        "/rig/morph", {"scene": name, "seconds": float(seconds), "curve": curve}
    )


def set_intensity(v):
    return post("/rig/intensity", {"value": max(0.0, min(1.2, float(v)))})


def set_metrics_link(strength):
    return post("/rig/metrics-link", {"strength": max(0.0, min(1.0, float(strength)))})


def macro(color=None, space=None, motion=None, crunch=None):
    m = {}
    if color is not None:
        m["macro.color"] = float(color)
    if space is not None:
        m["macro.space"] = float(space)
    if motion is not None:
        m["macro.motion"] = float(motion)
    if crunch is not None:
        m["macro.crunch"] = float(crunch)
    return post("/rig/param", {"batch": m})


def white_bloom(ms=800):
    return post("/rig/bloom", {"latch_ms": int(ms)})


def blackout(state=True):
    return post("/rig/blackout", {"state": bool(state)})
...
def movement_i_adagio(duration=30.0):
    ...
    set_intensity(0.20)
    macro(color=0.06, space=0.35, motion=0.15, crunch=0.0)
    t0 = time.time()
...
def movement_iii_dawn(duration=30.0):
    ...
    set_intensity(0.60)
    macro(color=0.14, space=0.75, motion=0.22, crunch=0.05)
    time.sleep(6.0)
    # resolution arc
...
def run_show(total=90, seed=None):
    ...
    try:
        post("/rig/param", {"key": "seed.value", "value": int(seed) if seed else 0})
    except:
        pass
...
if __name__ == "__main__":
    ...
    if args.mode == "jam":
        ...
        run_show(total=args.duration, seed=args.seed)
