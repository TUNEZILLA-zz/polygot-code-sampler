...
class LOLcatHUD:
    ...
    def render_hud(self):
        ...
        emoji_bar = "█" * int(self.emoji_density * 50) + "░" * (
            50 - int(self.emoji_density * 50)
        )
        ...
        chaos_bar = "█" * int(self.chaos_level * 50) + "░" * (
            50 - int(self.chaos_level * 50)
        )
        print(f"🌀 Chaos Level:   {chaos_color} {self.chaos_level:.3f} [{chaos_bar}]")

        # Trail intensity
        trail_color = self.get_status_color(
            self.trail_intensity, self.trail_warning, 0.6
        )
        trail_bar = "█" * int(self.trail_intensity * 50) + "░" * (
            50 - int(self.trail_intensity * 50)
        )
        print(
            f"🌈 Trail:        {trail_color} {self.trail_intensity:.3f} [{trail_bar}]"
        )
...
def main():
    ...
    parser.add_argument(
        "--preset", help="Test preset (classic, uwu-rainbow, stage-punch, cat-walk)"
    )
