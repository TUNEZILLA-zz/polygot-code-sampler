...
class BenchmarkSuite:
    ...
    def benchmark_parsing(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
        ...
        for code, name in test_cases:
            ...
            results["parse_times"].append({
                "case": name,
                "time_ms": parse_time * 1000,
                "memory_mb": mem_parse
            })
            
            results["ir_generation_times"].append({
                "case": name,
                "time_ms": parse_time * 1000  # Same as parsing for now
            })
            
            results["type_inference_times"].append({
                "case": name,
                "time_ms": infer_time * 1000,
                "memory_mb": mem_infer
            })
        
        # Calculate averages
        results["avg_parse_time_ms"] = sum(r["time_ms"] for r in results["parse_times"]) / len(results["parse_times"])
        results["avg_infer_time_ms"] = sum(r["time_ms"] for r in results["type_inference_times"]) / len(results["type_inference_times"])
        
        return results

    def benchmark_rust_generation(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
        ...
        for code, name in test_cases:
            ...
            # Parallel generation
            start_time = time.perf_counter()
            rust_parallel = render_rust(ir, func_name=name, parallel=True, type_info=type_info)
        ...
        results["avg_generation_time_ms"] = sum(r["time_ms"] for r in results["generation_times"]) / len(results["generation_times"])
        results["avg_parallel_generation_time_ms"] = sum(r["time_ms"] for r in results["parallel_generation_times"]) / len(results["parallel_generation_times"])
        
        return results

    def benchmark_typescript_generation(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
    ...
    def benchmark_sql_generation(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
    ...
    def benchmark_go_generation(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
    ...
    def benchmark_go_parallel_generation(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
    ...
    def benchmark_rust_execution(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
...
"""
        ...
        if successful_compiles:
            results["avg_compilation_time_ms"] = sum(r["time_ms"] for r in successful_compiles) / len(successful_compiles)
        if successful_execs:
            results["avg_execution_time_ms"] = sum(r["time_ms"] for r in successful_execs) / len(successful_execs)
        
        return results

    def benchmark_typescript_execution(self, test_cases: List[Tuple[str, str]]) -> Dict[str, Any]:
...
"""
        ...
        if successful_execs:
            results["avg_execution_time_ms"] = sum(r["time_ms"] for r in successful_execs) / len(successful_execs)
            results["avg_node_time_ns"] = sum(r["node_time_ns"] for r in successful_execs) / len(successful_execs)
    ...
    def run_full_benchmark(self) -> Dict[str, Any]:
        ...
        test_cases = [
            ...
            ("pairs = {(i, j) for i in range(3) for j in range(3) if i != j}", "nested_pairs"),
    ...
    def print_summary(self):
        ...
        print(f"   Rust (parallel): {rust_gen['avg_parallel_generation_time_ms']:.2f} ms")
        ...
        if "rust_execution" in self.results and "avg_compilation_time_ms" in self.results["rust_execution"]:
            rust_exec = self.results["rust_execution"]
            print(f"\n🦀 Rust Execution:")
            print(f"   Average compilation: {rust_exec['avg_compilation_time_ms']:.2f} ms")
            print(f"   Average execution: {rust_exec['avg_execution_time_ms']:.2f} ms")
        
        if "typescript_execution" in self.results and "avg_execution_time_ms" in self.results["typescript_execution"]:
            ts_exec = self.results["typescript_execution"]
            print(f"\n📘 TypeScript Execution:")
            print(f"   Average execution: {ts_exec['avg_execution_time_ms']:.2f} ms")
            print(f"   Average Node.js time: {ts_exec['avg_node_time_ns'] / 1_000_000:.2f} ms")
        ...
        if "scalability" in self.results:
            ...
            for case in scalability["cases"]:
                print(f"   {case['range_size']}: parse={case['parse_time_ms']:.2f}ms, gen={case['generation_time_ms']:.2f}ms")


def main():
    """Main benchmark runner"""
    parser = argparse.ArgumentParser(description="Polyglot Code Sampler Performance Benchmarks")
    parser.add_argument("--output", "-o", default="benchmark_results.json", help="Output file for results")
    parser.add_argument("--quick", action="store_true", help="Run quick benchmark (skip execution tests)")
    ...
    try:
        if args.quick:
            ...
            suite.results = {
                ...
                "typescript_generation": suite.benchmark_typescript_generation(test_cases),
                "sql_generation": suite.benchmark_sql_generation(test_cases),
                "go_generation": suite.benchmark_go_generation(test_cases),
                "go_parallel_generation": suite.benchmark_go_parallel_generation(test_cases),
            }
        else:
