...
class CosmicDustEngine:
    ...
    def apply_cosmic_dust(self, text: str, dust_type: CosmicDustType, params: CosmicDustParams) -> str:
    ...
    def apply_spectrum_dust(self, text: str, params: CosmicDustParams) -> str:
        ...
        for i, char in enumerate(text):
            ...
            if random.random() < params.density:
                ...
                spectrum_index = int((i * params.spectrum_shimmer) % len(self.rgb_spectrum))
                rgb = self.rgb_spectrum[spectrum_index]
                
                # Apply polarity shift
                if params.polarity_shift > 0:
                    # Positive polarity - bright colors
                    rgb = tuple(min(255, int(c * (1 + params.polarity_shift))) for c in rgb)
                elif params.polarity_shift < 0:
                    # Negative polarity - dark colors
                    rgb = tuple(max(0, int(c * (1 + params.polarity_shift))) for c in rgb)
                
                # Apply saturation
                if params.saturation < 1.0:
                    # Desaturate
                    gray = int(sum(rgb) / 3)
                    rgb = tuple(int(c * params.saturation + gray * (1 - params.saturation)) for c in rgb)
                
                # Add dust with RGB color
                result.append(f"\033[38;2;{rgb[0]};{rgb[1]};{rgb[2]}m{dust_char}\033[0m")
    ...
    def create_cosmic_preset(self, name: str, dust_type: CosmicDustType, params: CosmicDustParams) -> Dict[str, Any]:
