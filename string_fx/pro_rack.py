...
class ProRack:
    ...
    def morph_to(self, target_scene: RackScene, t: float) -> RackScene:
        ...
        morphed_fx = []
        for i, (current_fx, target_fx) in enumerate(
            zip(self.current_scene.fx, target_scene.fx)
        ):
            if current_fx.name == target_fx.name:
                ...
                for key in current_fx.params:
                    if key in target_fx.params:
                        morphed_params[key] = (
                            current_fx.params[key] * (1 - t) + target_fx.params[key] * t
                        )
                    else:
    ...
    def process(self, text: str, mode: str = "raw") -> str:
        ...
        if elapsed_ms > self.clamps["perf_budget_ms"]:
            print(
                f"⚠️  Performance warning: {elapsed_ms:.1f}ms > {self.clamps['perf_budget_ms']}ms"
            )
    ...
    def _apply_safety_clamps(self, fx_list: List[FXConfig]) -> List[FXConfig]:
        ...
        for fx in fx_list:
            ...
            if "feedback" in new_params:
                new_params["feedback"] = min(
                    self.clamps["feedback_max"], new_params["feedback"]
                )
            if "offset_px" in new_params:
                new_params["offset_px"] = min(
                    self.clamps["chromatic_offset_max"], new_params["offset_px"]
                )
            if "trail_len" in new_params:
                new_params["trail_len"] = min(
                    self.clamps["trail_max"], new_params["trail_len"]
                )
        ...
        for band_type, band_text in bands.items():
            if band_text:
                ...
                if band_type == BandType.SYMBOLS:
                    ...
                    band_fx = [
                        fx for fx in fx_list if fx.name in ["neon_fx", "chromatic"]
                    ]
                ...
                processed_bands[band_type] = self._process_serial(
                    band_text, band_fx, mode
                )
            else:
    ...
    def _recombine_bands(
        self, processed_bands: Dict[BandType, str], original: str
    ) -> str:
