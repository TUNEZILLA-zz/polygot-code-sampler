...
def render_go(ir: IRComp, func_name: str = "program", parallel: bool = False, type_info=None) -> str:
    """
    ...
    if len(ir.generators) == 1:
        ...
        if parallel:
            ...
            lines.append(f'    numWorkers := runtime.NumCPU()')
            lines.append(f'    chunkSize := ({stop} - {start}) / numWorkers')
            lines.append(f'    if chunkSize == 0 {{ chunkSize = 1 }}')
            lines.append('')
            ...
            lines.append(f'            end := start + chunkSize')
            lines.append(f'            if workerID == numWorkers-1 {{ end = {stop} }}')
            lines.append('')
            lines.append('            acc := 0')
            lines.append(f'            for {var} := start; {var} < end; {var} += {step} {{')
            ...
            if ir.reduce:
                ...
                if ir.kind == "list":
                    ...
                    lines.append(f'                acc += 1')  # Count unique elements
                elif ir.kind == "dict":
                    if ir.element:
                        lines.append(f'                acc += {ir.element}')
                    else:
            ...
            if ir.reduce:
                ...
                lines.append(f'    for {var} := {start}; {var} < {stop}; {var} += {step} {{')
                ...
                # Collection operations
                if ir.kind == "list":
                    lines.append(f'    result := make([]int, 0)')
                    lines.append(f'    for {var} := {start}; {var} < {stop}; {var} += {step} {{')
                    ...
                    lines.append(f'    result := make(map[int]struct{{}})')
                    lines.append(f'    for {var} := {start}; {var} < {stop}; {var} += {step} {{')
                    ...
                    lines.append(f'    result := make(map[int]int)')
                    lines.append(f'    for {var} := {start}; {var} < {stop}; {var} += {step} {{')
