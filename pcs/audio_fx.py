...
class LoopFxRewriter(ast.NodeTransformer):
    ...
    def _apply_delay(self, node: ast.For) -> ast.For:
        ...
        # Add delay buffer initialization before loop (not used in current implementation)
                        ...
                        value=ast.Call(
                            ...
                            args=[
                                ast.Call(
                                    func=ast.Attribute(
                                        value=ast.Name(
                                            id=delay_buffer_name, ctx=ast.Load()
                                        ),
                                        attr="pop",
                                        ctx=ast.Load(),
                                    ),
    ...
    def _apply_chorus(self, node: ast.For) -> ast.For:
        ...
        for voice in range(1, voices + 1):
            ...
            jittered_index = ast.BinOp(
                left=node.target, op=ast.Add(), right=ast.Constant(value=voice * jitter)
            )
            ...
            if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call):
                ...
                distorted_call = ast.Expr(
                    value=ast.Call(
                        ...
                        args=[
                            stmt.value,
                            ast.Constant(value=gain),
                            ast.Constant(value=clip),
                        ],
                        keywords=[],
                    )
                )
    ...
    def _apply_lfo(self, node: ast.For) -> ast.For:
        ...
        # target = self.fx_params.lfo_target  # Not used in current implementation
        freq = self.fx_params.lfo_freq
        # depth = self.fx_params.lfo_depth  # Not used in current implementation
    ...
    def _apply_swing(self, node: ast.For) -> ast.For:
        ...
        # ratio = self.fx_params.swing_ratio  # Not used in current implementation

        # Create swing condition
        swing_condition = ast.If(
            test=ast.Compare(
                left=ast.BinOp(
                    left=node.target, op=ast.Mod(), right=ast.Constant(value=2)
                ),
                ops=[ast.Eq()],
...
class AudioFxEngine:
    ...
    def __init__(self):
        self.default_safety = {"max_expand": 8, "max_cost_multiplier": 3.0}
    ...
    def create_fx_spec(
        ...
        return FxSpec(chain=fx_chain, params=params, safety=safety)
    ...
    def validate_fx_spec(self, fx_spec: FxSpec) -> Tuple[bool, List[str]]:
        ...
        if total_expand > max_expand:
            issues.append(
                f"Total expansion ({total_expand}) exceeds max_expand ({max_expand})"
            )
...
"""
    ...
    reverb_spec = engine.create_fx_spec(
        fx_chain=["reverb"], params={"reverb_taps": 3, "reverb_decay": 0.6}
    )
    ...
    delay_spec = engine.create_fx_spec(
        fx_chain=["delay"], params={"delay_steps": 3, "delay_extend_loop": True}
    )
    ...
    reverse_spec = engine.create_fx_spec(fx_chain=["reverse"], params={})
    ...
    chorus_spec = engine.create_fx_spec(
        fx_chain=["chorus"], params={"chorus_voices": 3, "chorus_jitter": 0.05}
    )
    ...
    chain_spec = engine.create_fx_spec(
        fx_chain=["reverse", "reverb"], params={"reverb_taps": 2, "reverb_decay": 0.5}
    )
