...
def generate_showcase_chart():
    ...
    csharp_sequential = render_csharp(
        ir, func_name="CalculateCompletedOrdersSequential"
    )
    csharp_parallel = render_csharp(
        ir, func_name="CalculateCompletedOrdersParallel", parallel=True
    )
    sql_sqlite = render_sql(
        ir, func_name="calculate_completed_orders", dialect="sqlite"
    )
    sql_postgresql = render_sql(
        ir, func_name="calculate_completed_orders", dialect="postgresql"
    )
